...
static const int32_t zetas[MLDSA44_N] = {
...
*
* Arguments:   - uint32_t p[MLDSA44_N]: input/output coefficient array
**************************************************/
void PQCLEAN_MLDSA44_CLEAN_ntt(int32_t a[MLDSA44_N]) {
    unsigned int len, start, j, k;
    int32_t zeta, t;

    k = 0;
    for (len = 128; len > 0; len >>= 1) {
        for (start = 0; start < MLDSA44_N; start = j + len) {
            zeta = zetas[++k];
...
* Arguments:   - uint32_t p[MLDSA44_N]: input/output coefficient array
**************************************************/
void PQCLEAN_MLDSA44_CLEAN_invntt_tomont(int32_t a[MLDSA44_N]) {
    unsigned int start, len, j, k;
    int32_t t, zeta;
    const int32_t f = 41978; // mont^2/256

    k = 256;
    for (len = 1; len < MLDSA44_N; len <<= 1) {
        for (start = 0; start < MLDSA44_N; start = j + len) {
            zeta = -zetas[--k];
            for (j = start; j < start + len; ++j) {
                t = a[j];
...
void PQCLEAN_MLDSA44_CLEAN_poly_uniform(poly *a,
    ...
    ctr = rej_uniform(a->coeffs, MLDSA44_N, buf, buflen);
    ...
    while (ctr < MLDSA44_N) {
        ...
        ctr += rej_uniform(a->coeffs + ctr, MLDSA44_N - ctr, buf, buflen);
    }
...
void PQCLEAN_MLDSA44_CLEAN_poly_uniform_eta(poly *a,
    ...
    ctr = rej_eta(a->coeffs, MLDSA44_N, buf, buflen);

    while (ctr < MLDSA44_N) {
        stream256_squeezeblocks(buf, 1, &state);
        ctr += rej_eta(a->coeffs + ctr, MLDSA44_N - ctr, buf, STREAM256_BLOCKBYTES);
    }
...
void PQCLEAN_MLDSA44_CLEAN_poly_challenge(poly *c, const uint8_t seed[CTILDEBYTES]) {
    ...
    for (i = MLDSA44_N - TAU; i < MLDSA44_N; ++i) {
        do {
...
void PQCLEAN_MLDSA44_CLEAN_polyvec_matrix_expand(polyvecl mat[MLDSA44_K], const uint8_t rho[SEEDBYTES]) {
    unsigned int i, j;

    for (i = 0; i < MLDSA44_K; ++i) {
        for (j = 0; j < MLDSA44_L; ++j) {
...
void PQCLEAN_MLDSA44_CLEAN_polyvec_matrix_pointwise_montgomery(polyveck *t, const polyvecl mat[MLDSA44_K], const polyvecl *v) {
    unsigned int i;
...
void PQCLEAN_MLDSA44_CLEAN_polyvecl_uniform_gamma1(polyvecl *v, const uint8_t seed[CRHBYTES], uint16_t nonce) {
    ...
    for (i = 0; i < MLDSA44_L; ++i) {
        PQCLEAN_MLDSA44_CLEAN_poly_uniform_gamma1(&v->vec[i], seed, (uint16_t) (MLDSA44_L * nonce + i));
    }
}
...
*
* Description: Add vectors of polynomials of length MLDSA44_L.
...
*
* Description: Forward NTT of all polynomials in vector of length MLDSA44_L. Output
...
*
* Description: Pointwise multiply vectors of polynomials of length MLDSA44_L, multiply
...
*
* Description: Check infinity norm of polynomials in vector of length MLDSA44_L.
...
/**************************************************************/
/************ Vectors of polynomials of length MLDSA44_K **************/
/**************************************************************/
...
*
* Description: Reduce coefficients of polynomials in vector of length MLDSA44_K
...
*
* Description: For all coefficients of polynomials in vector of length MLDSA44_K
...
*
* Description: Add vectors of polynomials of length MLDSA44_K.
*              No modular reduction is performed.
*
...
*
* Description: Subtract vectors of polynomials of length MLDSA44_K.
*              No modular reduction is performed.
*
* Arguments:   - polyveck *w: pointer to output vector
...
*
* Description: Multiply vector of polynomials of Length MLDSA44_K by 2^D without modular
...
*
* Description: Forward NTT of all polynomials in vector of length MLDSA44_K. Output
*              coefficients can be up to 16*Q larger than input coefficients.
*
* Arguments:   - polyveck *v: pointer to input/output vector
**************************************************/
...
*              in vector of length MLDSA44_K. Input coefficients need to be less
*              than 2*Q.
*
* Arguments:   - polyveck *v: pointer to input/output vector
**************************************************/
...
*
* Description: Check infinity norm of polynomials in vector of length MLDSA44_K.
...
*
* Description: For all coefficients a of polynomials in vector of length MLDSA44_K,
...
void PQCLEAN_MLDSA44_CLEAN_polyveck_pack_w1(uint8_t r[MLDSA44_K * POLYW1_PACKEDBYTES], const polyveck *w1) {
    unsigned int i;
...
void PQCLEAN_MLDSA44_CLEAN_pack_sig(uint8_t sig[PQCLEAN_MLDSA44_CLEAN_CRYPTO_BYTES],
    ...
    sig += MLDSA44_L * POLYZ_PACKEDBYTES;

    /* Encode h */
    for (i = 0; i < OMEGA + MLDSA44_K; ++i) {
        sig[i] = 0;
    }
...
int PQCLEAN_MLDSA44_CLEAN_crypto_sign_signature_ctx(uint8_t *sig, size_t *siglen, const uint8_t *m, size_t mlen, const uint8_t *ctx, size_t ctxlen, const uint8_t *sk) {
    unsigned int n;
    uint8_t seedbuf[2 * SEEDBYTES + TRBYTES + RNDBYTES + 2 * CRHBYTES];     // ! rhoprime에 hedged 부분을 위해 rndbytes가 추가됨 H( MLDSA44_K | tr ) -> H( MLDSA44_K | rnd | tr), deterministic일때는 rnd대신 all zero값이 들어가서 연산됨
    uint8_t *rho, *tr, *key, *mu, *rhoprime, *rnd;                          // ! rnd부분이 추가됨
    uint16_t nonce = 0;
    polyvecl mat[MLDSA44_K], s1, y, z;
    ...
    rho = seedbuf;
    tr = rho + SEEDBYTES;
    key = tr + TRBYTES;                 // ! SEEDBYTES -> TRBYTES
    rnd = key + SEEDBYTES;
    mu = rnd + RNDBYTES;                // ! 해당 부분이 추가
    ...
    shake256_inc_init(&state);
    shake256_inc_absorb(&state, tr, TRBYTES);   // H(tr)                 ! SEEDBYTES -> TRBYTES
    shake256_inc_absorb(&state, mu, 2);         // H(tr | 0 | ctxlen)
    shake256_inc_absorb(&state, ctx, ctxlen);   // H(tr | 0 | ctxlen | ctx)
    shake256_inc_absorb(&state, m, mlen);       // H(tr | 0 | ctxlen | ctx | m)
    shake256_inc_finalize(&state);
    shake256_inc_squeeze(mu, CRHBYTES, &state); // mu = H(tr | 0 | ctxlen | ctx | m)
    shake256_inc_ctx_release(&state);


    randombytes_win32_randombytes(rnd, RNDBYTES);   // ! hedged를 위한 rnd 생성

    shake256(rhoprime, CRHBYTES, key, SEEDBYTES + RNDBYTES + CRHBYTES); // rhoprime = H (K | rnd | mu)

    /* Expand matrix and transform vectors */
    PQCLEAN_MLDSA44_CLEAN_polyvec_matrix_expand(mat, rho);
    PQCLEAN_MLDSA44_CLEAN_polyvecl_ntt(&s1);    // s1 NTT 변환
    PQCLEAN_MLDSA44_CLEAN_polyveck_ntt(&s2);    // s2 NTT 변환
    PQCLEAN_MLDSA44_CLEAN_polyveck_ntt(&t0);    // t0 NTT 변환
...
int main()
{
    ...
    size_t mlen, smlen;
    uint8_t b;
    uint8_t m[MLEN + PQCLEAN_MLDSA44_CLEAN_CRYPTO_BYTES];    // 실제         message
    uint8_t m2[MLEN + PQCLEAN_MLDSA44_CLEAN_CRYPTO_BYTES];   // verify에 있는 message
    uint8_t sm[MLEN + PQCLEAN_MLDSA44_CLEAN_CRYPTO_BYTES];   // sign에 있는  message
    ...
    PQCLEAN_MLDSA44_CLEAN_crypto_sign_ctx(sm, &smlen, m, MLEN, ctx, CTX_LEN, sk);
    ret = PQCLEAN_MLDSA44_CLEAN_crypto_sign_open_ctx(m2, &mlen, sm, smlen, ctx, CTX_LEN, pk);
    ...
    if (smlen != MLEN + PQCLEAN_MLDSA44_CLEAN_CRYPTO_BYTES)
    {
    ...
    sm[j % (MLEN + PQCLEAN_MLDSA44_CLEAN_CRYPTO_BYTES)] += b;
    ret = PQCLEAN_MLDSA44_CLEAN_crypto_sign_open_ctx(m2, &mlen, sm, smlen, ctx, CTX_LEN, pk);
    if (!ret)
    {
