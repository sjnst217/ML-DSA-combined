...
#define SEEDBYTES 32            // 256bit 짜리 random 값을 구할 때 이용 & A, s1, s2, MLDSA44_K 의 seed를 생성하는 seed, 
...
#define MLDSA44_N 256         // 다항식의 최대 차수
#define Q 8380417               // modulo Q
#define D 13                    // t = t1*2^D+t0 를 구할 때 사용하는 즉, t의 high bit를 구하기 위한 값
#define ROOT_OF_UNITY 1753      // NTT에서 이용하는 MLDSA44의 root of unity

#define MLDSA44_K 4           // 공개행렬 A에서 행의 크기
#define MLDSA44_L 4           // 공개행렬 A에서 열의 크기
...
#define CTILDEBYTES 32          // tilde c의 범위 설정
...
각 계수들은 MLDSA44의 보안레벨에 따라 달라지기 때문에 packing 할 때의 크기 역시 보안 레벨에 따라 달라지게 됨
*/


#define POLY_UNIFORM_NBLOCKS ((768 + STREAM128_BLOCKBYTES - 1)/STREAM128_BLOCKBYTES)                            // MLDSA44의 계수는 23bit 표현이기 때문에, (256 * 24 / 8 +  168 - 1) / 168 = 5
...
#define POLYVECH_PACKEDBYTES (OMEGA + MLDSA44_K)        // h            hint vector h는 계수가 1 또는 0이기 때문에 일반적인 encoding과 다를 수 있음. 
...
#define PQCLEAN_MLDSA44_CLEAN_CRYPTO_PUBLICKEYBYTES (SEEDBYTES + MLDSA44_K*POLYT1_PACKEDBYTES)
...
#define PQCLEAN_MLDSA44_CLEAN_CRYPTO_SECRETKEYBYTES (2*SEEDBYTES \
        + TRBYTES \
        + MLDSA44_L*POLYETA_PACKEDBYTES \
        + MLDSA44_K*POLYETA_PACKEDBYTES \
        + MLDSA44_K*POLYT0_PACKEDBYTES)



#define PQCLEAN_MLDSA44_CLEAN_CRYPTO_BYTES (CTILDEBYTES + MLDSA44_L*POLYZ_PACKEDBYTES + POLYVECH_PACKEDBYTES)  // packing 했을 때의 signature의 크기:  challenge 벡터를 생성하는 seed + z의 다항식 벡터 + h의 다항식 벡터

typedef struct {
        int32_t coeffs[MLDSA44_N];
} poly;
